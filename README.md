...
- Video: https://youtu.be/R-WpQ-6_KME (4:38). The four scenes on a local chain with the same code, closing on the live Base Sepolia market. The repo's copy, [media/bazaar-demo.mp4](media/bazaar-demo.mp4) (4:36), is a later take that corrects two notes: an arbiter upholding a dispute also counts as confirmed, and the run counts in the captions come from the agents' logs.
